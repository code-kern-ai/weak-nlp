...
for text_id, text in enumerate(texts):
    ...
    if label is not None:
        association = weak_nlp.SingleClassificationAssociation(text_id + 1, label)
        lf_associations.append(association)

lf_vector = weak_nlp.SourceVector(contains_keywords.__name__, False, lf_associations)

ground_truths = [
    weak_nlp.SingleClassificationAssociation(1, "clickbait"),
    weak_nlp.SingleClassificationAssociation(2, "regular"),
    weak_nlp.SingleClassificationAssociation(3, "regular")
]

gt_vector = weak_nlp.SourceVector("ground_truths", True, ground_truths)

cnlm = weak_nlp.SingleLabelClassificationNLM([gt_vector, lf_vector])
```
...
for text_id, text in enumerate(texts):
    for triplet in match_keywords(text):
        ...
        association = weak_nlp.ExclusiveEntityExtractionAssociation(text_id + 1, label, from_idx, to_idx)
        lf_associations.append(association)

lf_vector = weak_nlp.SourceVector(match_keywords.__name__, False, lf_associations)

ground_truths = [
    weak_nlp.ExclusiveEntityExtractionAssociation(1, "person", 1, 2),
    weak_nlp.ExclusiveEntityExtractionAssociation(2, "person", 4, 5),
]

gt_vector = weak_nlp.SourceVector("ground_truths", True, ground_truths)

enlm = weak_nlp.ExclusiveEntityExtractionNLM([gt_vector, lf_vector])
```
